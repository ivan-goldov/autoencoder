...
def get_train_data(folder_path: str) -> Dict[str, Any]:
    train_paths = [
        join(folder_path, f) for f in listdir(folder_path) if f.startswith('data_batch')
    ]

    train_dicts = [unpickle(f) for f in train_paths]
    convenient_train_dict = {'data': [], 'labels': []}
    for d in train_dicts:
        convenient_train_dict['data'].extend(d[b'data'])
        convenient_train_dict['labels'].extend(d[b'labels'])
    return convenient_train_dict


def get_test_data(folder_path: str) -> Dict[str, Any]:
    test_batch = join(folder_path, 'test_batch')
    test_dict = unpickle(test_batch)
    convenient_test_dict = {'data': test_dict[b'data'], 'labels': test_dict[b'labels']}
